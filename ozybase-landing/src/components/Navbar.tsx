  ...
  return (
    <nav className="sticky top-0 w-full z-50 bg-background/80 backdrop-blur-md border-b border-border-subtle">
      <div className="flex justify-between items-center h-16 px-6 md:px-16 max-w-[1280px] mx-auto">
        {/* Logo */}
        <a href="#" className="flex items-center gap-2 font-headline text-2xl font-bold tracking-tighter text-white">
        ...
        <div className="hidden md:flex gap-8 items-center">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-terminal-gray font-mono text-sm tracking-wider hover:text-success-neon transition-colors duration-200"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="md:hidden absolute top-16 left-0 w-full bg-background/95 backdrop-blur-md border-b border-border-subtle flex flex-col p-6 gap-6 z-40">
          {navLinks.map((link) => (
            <a
              ...
              className="text-terminal-gray font-mono text-base tracking-wider hover:text-success-neon transition-colors"
            >
              {link.name}
            </a>
          ))}
